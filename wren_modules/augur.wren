class Assert {
  ...
  static notEqual(actual, expected){
    if(actual == expected){
      Fiber.abort("%(actual) was NOT expected to be %(expected)")
    }
  }
  ...
  static notNull(actual){
    if(actual == null){
      Fiber.abort("Expected value to be not null")
    }
  }

  static isNull(actual){
    if(actual != null){
      Fiber.abort("Expected value to be not null")
    }
  }
  ...
  static isTrue(val){
    if(!val) Fiber.abort("Expected value to be true")
  }

  static isFalse(val){
    if(val) Fiber.abort("Expected value to be false")
  }

  static fail(){
    Fiber.abort("Reached fail condition")
  }

  static instanceOf(instance, type){
    if(!instance is type) Fiber.abort("%(instance) is not of type %(type)")
  }
}
